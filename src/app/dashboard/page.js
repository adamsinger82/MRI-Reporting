...
FINDINGS RULES:
1. Structures not mentioned in dictation: write "intact" only — nothing else.
...
IMPRESSION RULES:
- Synthesize all positive findings into a clinically meaningful, concise impression.
- Group related findings under a unifying diagnosis where appropriate (e.g. multiple findings from a pivot shift injury grouped together).
- Number each impression item. Most important first.
- Use standard MSK radiology impression language — concise, diagnostic.
- If entirely normal: "No significant MRI findings of the ${lat ? lat + ' ' : ''}${part}."

FORMAT — exact section headers on their own lines in ALL CAPS:

TECHNIQUE:
Multiplanar multisequence MRI of the ${lat ? lat + ' ' : ''}${part} ${con} IV contrast.

FINDINGS:
[Title Case subheading: finding — one per structure]
...
export default function DashboardPage() {
        ...
        if (recognitionRef.current === recognition) {
          setTimeout(() => {
            ...
            try {
              ...
              rec2.onerror = recognition.orerror;
  ...
  const inp = { width: '100%', padding: '9px 12px', border: '1px solid #dde3ed', borderRadius: '8px', fontSize: '14px', boxSizing: 'border-box', color: '#1e293b', outline: 'none', background: 'white' };
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(160deg,#0d1b2a 0%,#1a3a5c 45%,#0d1b2a 100%)', fontFamily: "'Segoe UI',system-ui,sans-serif" }}>
      ...
      {/* ── 3-column grid — inline styles, cannot be overridden ── */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16, padding: 16, boxSizing: 'border-box' }}>
        ...
        <div style={{ background: 'white', borderRadius: 16, overflow: 'hidden', boxShadow: '0 4px 24px rgba(0,0,0,0.18)', display: 'flex', flexDirection: 'column' }}>
          <div style={{ background: 'linear-gradient(135deg,#1d4ed8,#2563eb)', padding: '13px 16px', display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ color: 'white', fontWeight: 700, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.12em' }}>Dictation Input</span>
          </div>
          <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12, flex: 1 }}>
            ...
            <div style={{ padding: '9px 12px', background: 'linear-gradient(135deg,#eff6ff,#f0f9ff)', borderRadius: 8, border: '1px solid #bfdbfe', fontSize: 12, color: '#1d4ed8', fontStyle: 'italic', lineHeight: 1.5 }}>
              {technique}
            </div>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
              ...
              <textarea
                style={{ ...inp, flex: 1, minHeight: 160, resize: 'vertical', lineHeight: 1.7, fontFamily: 'inherit', border: isListening ? '1.5px solid #ef4444' : '1px solid #dde3ed', boxShadow: isListening ? '0 0 0 3px rgba(239,68,68,0.1)' : 'none', transition: 'all 0.15s' }}
                value={dictationText}
            ...
            <button onClick={isListening ? stopListening : toggleListening}
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, width: '100%', padding: '10px', borderRadius: 9, border: '1.5px solid ' + (isListening ? '#fca5a5' : '#dde3ed'), background: isListening ? '#fef2f2' : '#f8fafc', fontSize: 14, fontWeight: 600, cursor: 'pointer', color: isListening ? '#dc2626' : '#475569', transition: 'all 0.15s' }}>
        ...
        <div style={{ background: 'white', borderRadius: 16, overflow: 'hidden', boxShadow: '0 4px 24px rgba(0,0,0,0.18)', display: 'flex', flexDirection: 'column' }}>
          <div style={{ background: 'linear-gradient(135deg,#5b21b6,#7c3aed)', padding: '13px 16px', display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 15 }}>📄</span>
            <span style={{ color: 'white', fontWeight: 700, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.12em' }}>Generated Report</span>
          </div>
          <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12, flex: 1 }}>
            <div style={{ flex: 1, padding: '14px 16px', border: '1px solid #e8edf5', borderRadius: 10, overflowY: 'auto', minHeight: 340, maxHeight: '65vh', background: generatedReport ? 'white' : '#f8fafc' }}>
              {isGenerating
                ? <div style={{ display: 'flex', flexDirection: 'column', gap: 10, paddingTop: 4 }}>
                    {[55, 80, 65, 90, 50, 72, 60].map((w, i) => (
                      <div key={i} style={{ height: 9, background: `rgba(37,99,235,${0.06 + i * 0.02})`, borderRadius: 4, width: w + '%' }} />
                    ))}
                  </div>
                : generatedReport
                  ? <div style={{ fontFamily: "Georgia,'Times New Roman',serif" }}>{formatReport(generatedReport)}</div>
                  : <div style={{ color: '#94a3b8', fontStyle: 'italic', fontSize: 13, paddingTop: 4, lineHeight: 1.8, textAlign: 'center', paddingTop: 40 }}>
        ...
        <div style={{ background: 'white', borderRadius: 16, overflow: 'hidden', boxShadow: '0 4px 24px rgba(0,0,0,0.18)', display: 'flex', flexDirection: 'column' }}>
          <div style={{ background: 'linear-gradient(135deg,#0e7490,#0891b2)', padding: '13px 16px', display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 15 }}>📐</span>
            <span style={{ color: 'white', fontWeight: 700, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.12em' }}>Reference Panel</span>
          </div>
