...
const DIAGRAM_SVGS = {
  ...
  'knee-meniscal-extrusion': (
    <svg viewBox="0 0 320 220" className="w-full" aria-label="Meniscal extrusion measurement diagram">
      {/* Tibial plateau */}
...
function ReferencePanel({ selectedBodyPart }) {
  ...
  // Reset selected measurement when body part changes
  useEffect(() => {
    setSelectedMeasurementId('');
  }, [selectedBodyPart]);

  const selectedMeasurement = jointData?.measurements?.find(
    ...
    <div className="reference-panel">
      ...
      <div className="reference-top">
        <h3 className="ref-section-title">
          <span className="ref-section-icon">📐</span>
        ...
        {selectedMeasurement ? (
          <div className="measurement-detail">
            <div className="measurement-meta">
              <span className="meta-badge">{selectedMeasurement.plane}</span>
              <p className="measurement-description">{selectedMeasurement.description}</p>
            </div>
      ...
      <div className="reference-bottom">
        ...
        {selectedMeasurement ? (
          <table className="normal-values-table">
            <tbody>
              {selectedMeasurement.normalValues.map((nv, i) => (
                <tr key={i}>
                  <td className="nv-label">{nv.label}</td>
                  <td className="nv-value">{nv.value}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ) : (
          <div className="normal-values-all">
            {jointData.measurements.map((m) => (
              <div key={m.id} className="nv-summary-row">
                <button
                  className="nv-summary-name"
                  onClick={() => setSelectedMeasurementId(m.id)}
                >
                  {m.label}
                </button>
                <span className="nv-summary-first">
...
export default function DashboardPage() {
  ...
  const autoDetectBodyPart = async (text) => {
    ...
    try {
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'user',
              content: `Based on this radiology dictation, which body part is being described? Reply with ONLY one of these exact values: knee, shoulder, hip, wrist, elbow, ankle, spine, pelvis, foot. Dictation: "${text}"`,
            },
          ],
        }),
      });
      ...
      const valid = BODY_PARTS.map((b) => b.id);
      if (valid.includes(detected)) {
        setSelectedBodyPart(detected);
      }
    } catch (err) {
  ...
  const generateReport = async () => {
    ...
    const bodyPartLabel = BODY_PARTS.find((b) => b.id === selectedBodyPart)?.label || selectedBodyPart;
    ...
    try {
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          model: 'claude-sonnet-4-6',
          max_tokens: 1000,
          system: `You are an expert MSK radiologist assistant. Generate a professional, structured MRI radiology report for a ${bodyPartLabel} MRI based on the provided dictation notes. Use standard radiology report format with sections: TECHNIQUE, FINDINGS, and IMPRESSION. Use professional medical terminology. Be concise and clinically relevant.`,
          messages: [
            {
              role: 'user',
              content: `Generate an MRI report for: ${bodyPartLabel}\n\nDictation notes:\n${dictationText}`,
            },
          ],
        }),
      });
  ...
  const styles = `
    ...
    .body-part-select:focus { outline: none; border-color: #3b82f6; box-shadow: 0 0 0 3px rgba(59,130,246,0.12); }
    ...
    .dictation-area:focus { outline: none; border-color: #3b82f6; box-shadow: 0 0 0 3px rgba(59,130,246,0.12); }
    .dictation-area.listening { border-color: #ef4444; box-shadow: 0 0 0 3px rgba(239,68,68,0.12); }
    ...
    .btn-mic.active { background: #fef2f2; border-color: #fca5a5; color: #dc2626; }
    ...
    .btn-copy:hover:not(:disabled) { background: #f1f5f9; }
    .btn-copy.success { background: #f0fdf4; border-color: #86efac; color: #16a34a; }
    ...
    .reference-panel-empty { color: #94a3b8; font-size: 13px; }
    ...
    .ref-section-icon { font-size: 14px; }
    ...
    .measurement-select:focus { outline: none; border-color: #3b82f6; }

    .measurement-detail { display: flex; flex-direction: column; gap: 8px; }
    .measurement-meta { display: flex; flex-direction: column; gap: 4px; }
    .meta-badge {
    ...
    .measurement-description { font-size: 12px; color: #64748b; margin: 0; line-height: 1.5; }
    ...
    .measurement-empty, .reference-panel-empty {
      padding: 12px;
      background: #f8fafc;
      border-radius: 8px;
      color: #94a3b8;
    ...
    .normal-values-table { width: 100%; border-collapse: collapse; font-size: 12px; }
    .normal-values-table tr:not(:last-child) td { border-bottom: 1px solid #f1f5f9; }
    .nv-label { padding: 5px 4px; color: #64748b; width: 45%; vertical-align: top; }
    .nv-value { padding: 5px 4px; color: #1e293b; font-weight: 500; font-family: 'Courier New', monospace; }

    .normal-values-all { display: flex; flex-direction: column; gap: 6px; overflow-y: auto; max-height: 200px; }
    .nv-summary-row { display: flex; flex-direction: column; gap: 2px; padding: 6px 8px; background: #f8fafc; border-radius: 6px; }
    .nv-summary-name {
      font-size: 12px; font-weight: 600; color: #2563eb; background: none;
      border: none; cursor: pointer; text-align: left; padding: 0; text-decoration: underline;
    }
    .nv-summary-first { font-size: 11px; color: #64748b; }
      ...
      <div className="dashboard-layout">
        ...
        <div className="col">
          ...
          <div>
            ...
            <textarea
              className={`dictation-area${isListening ? ' listening' : ''}`}
              value={dictationText}
              onChange={(e) => {
          ...
          <button className={`btn-mic${isListening ? ' active' : ''}`} onClick={toggleListening}>
        ...
        {/* ─── COL 2: REPORT ─── */}
        <div className="col">
          <p className="col-title">Generated Report</p>

          <div className="report-output">
            {isGenerating ? (
              <span className="report-placeholder">Generating report…</span>
            ) : generatedReport ? (
              generatedReport
            ) : (
        ...
        {/* ─── COL 3: REFERENCE ─── */}
        <div className="col">
          <p className="col-title">Reference</p>
