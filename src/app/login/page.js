...
const ANATOMY_MRI = {
  ...
  shoulder:'Supraspinatus and Infraspinatus Tendons, Subscapularis Tendon, Teres Minor Tendon, Biceps Tendon Long Head, Acromioclavicular Joint, Glenoid Labrum, Acromial Undersurface, Subacromial and Subdeltoid Bursa, Articular Cartilage, Bones, Joint Effusion, Capsulitis, Muscles, Regional Neurovascular Structures, Soft Tissues',
...
// ─── BILATERAL BODY PART PLURALIZATION ──────────────────────────────────────
// Used in TECHNIQUE text when laterality is bilateral, e.g. "bilateral hands"
// not "bilateral hand". Handles irregular plurals explicitly.
const BODY_PART_PLURALS = {
  'foot': 'feet',
  'femur/thigh': 'femurs/thighs',
  'tibia/fibula': 'tibias/fibulas',
};
function pluralizeBodyPart(part) {
  if (BODY_PART_PLURALS[part]) return BODY_PART_PLURALS[part];
  if (/[sxz]$|[cs]h$/.test(part)) return part + 'es';
  return part + 's';
}
...
function buildPrompt(part, lat, con, spineRegion, modality, doseOpt = true, massMode = 'auto') {
  ...
  const isBilateral = (lat || '').toLowerCase() === 'bilateral';
  const techniquePartLabel = part === 'spine'
    ? spineRegion + ' spine'
    : (isBilateral ? pluralizeBodyPart(part) : part);
  const techniqueText = isCT
    ? `CT scan of the ${lat ? lat + ' ' : ''}${techniquePartLabel} ${con} IV contrast. Multiplanar reformats were created.${doseOptSentence}`
    : `Multiplanar multisequence MRI of the ${lat ? lat + ' ' : ''}${techniquePartLabel} ${con} IV contrast.`;

  const findingsRules = isCT
    ? `FINDINGS RULES (CT): 1. Not mentioned: write "intact." EXCEPTION: Joint Effusion, Dislocation or Subluxation — write "absent." Soft Tissues — write "No acute soft tissue abnormality." 2. Positive: exact dictated words only. 3. CT language only: attenuation, cortical integrity, trabecular pattern, osteophytes, subchondral cysts, chondrocalcinosis. No T1/T2/STIR language. 4. BONES RULE — use a SINGLE "Bones:" heading for ALL bone findings. If nothing is dictated, the entire Bones line should read exactly one sentence: "No acute fracture, AVN or suspicious lytic/blastic lesion." Do NOT also append a second/duplicate negative sentence about osteonecrosis or osseous lesion anywhere else in the Bones line — this default is a single, complete sentence, never a bundle of several negative sentences. If a specific finding IS dictated (e.g. a fracture, AVN, or a lytic/blastic/aggressive bone lesion — see the BONE LESION vs SOFT TISSUE MASS rule under MASS/TUMOR/CANCER RULES for how to describe a bone lesion here), describe that finding in place of its corresponding clause and keep the remaining negative language only for whatever was NOT dictated. Do NOT restate osteoarthrosis/joint space narrowing severity under Bones — see JOINT SPACE RULE below; OA-related findings are reported ONCE, under Joint Space only, never duplicated under Bones. CRITICAL: Do NOT create any subheading named after an individual bone (e.g. do NOT write "Tibia:", "Fibula:", "Femur:", "Patella:", "Calcaneus:", "Talus:", "Radius:", "Ulna:", "Humerus:", etc. as standalone findings headings). ALL bone findings — regardless of how many bones the study covers — consolidate under the single "Bones:" heading; describe each bone within that section's text as needed. 5. JOINT SPACE RULE — for each joint space: address narrowing, osteophytes, subchondral cysts, chondrocalcinosis — or write "Preserved joint space without osteophytes, subchondral cysts, or chondrocalcinosis." This is the SOLE location for osteoarthrosis/degenerative joint space findings — never also summarize OA severity under Bones. 6. ATHEROSCLEROSIS: report under Soft Tissues, never under Bones.`
    : `FINDINGS RULES: 1. Not mentioned: write "intact." EXCEPTION: Joint Effusion, Baker Cyst, bursae, soft tissue masses — write "absent" not "intact." 2. Positive: exact dictated words only, no added morphology/signal/measurements. This includes location/anatomic descriptors (e.g. "anterior leading edge," "posterior leading edge," "footprint," "watershed zone") — reproduce the dictated location phrase verbatim; never silently substitute it with a different anatomic term that seems more standard or more familiar, even if that other term is also a real anatomic structure in the same region. If you are not certain a substitution is a pure terminology/garble correction explicitly authorized elsewhere in this prompt, do not make it — use the dictated words. 3. BONES RULE — use a SINGLE heading "Bones:" for ALL bone findings. If nothing is dictated, the entire Bones line should read exactly one sentence: "No acute fracture, AVN or marrow infiltration." Do NOT also append a second/duplicate negative sentence about marrow infiltration, osteonecrosis, or bone lesion anywhere else in the Bones line — this default is a single, complete sentence, never a bundle of several negative sentences. If a specific finding IS dictated (e.g. a fracture, contusion, AVN, marrow infiltration, or a bone lesion — see the BONE LESION vs SOFT TISSUE MASS rule under MASS/TUMOR/CANCER RULES for how to describe a bone lesion here), describe that finding in place of its corresponding clause and keep the remaining negative language only for whatever was NOT dictated (e.g. if only a fracture is dictated, write the fracture finding followed by "No AVN or marrow infiltration."). BONES SCOPE: include acute fractures, bone contusions, stress reactions/stress fractures, AVN, bone lesions (benign or malignant — see BONE LESION vs SOFT TISSUE MASS rule for placement), and marrow infiltration. Do NOT include chronic degenerative changes (osteophytes, subchondral sclerosis/cysts from OA) — those belong under Articular Cartilage or joint headings. Atherosclerosis belongs under Soft Tissues, never under Bones. CRITICAL: Do NOT create any subheading named after an individual bone (e.g. do NOT write "Femur:", "Tibia:", "Patella:", "Fibula:", "Calcaneus:", "Talus:" etc. as standalone findings headings). All bone findings consolidate under the single "Bones:" heading.`;
...
- CLINICAL HISTORY IS CONTEXT ONLY — NEVER A FINDING SOURCE (ZERO TOLERANCE): If the dictation includes clinical history, reason-for-exam, or prior-treatment/procedure information (e.g. "history of failed SI joint injection," "here for follow-up of..."), that information exists purely for clinical context. NEVER use it to add, infer, invent, relocate, or embellish an imaging finding under any heading. Every finding in FINDINGS/LEVELS/IMPRESSION must come only from what the radiologist explicitly dictated as an actual imaging observation for that specific structure — never from what the history might lead you to expect to find there. An explicit dictated negative for a structure (e.g. "no acute finding in the SI joints," "SI joints unremarkable") must be reported exactly as that negative finding for that exact structure, regardless of what the clinical history mentions about that structure or a related prior procedure — never substitute, reinterpret, or relocate a different dictated finding (e.g. facet edema dictated at a lumbar level) onto the structure named in the history just because the history makes that structure seem clinically relevant.
...
NO TREATMENT/SURGICAL RECOMMENDATIONS OR UNSUPPORTED SEVERITY LABELS (applies everywhere in the report, not just the impression): Never state or imply that a finding "likely needs," "will likely require," or "may require" surgical intervention, or describe a patient as a "surgical candidate" — imaging alone does not establish a treatment plan, and this is a clinical/surgical judgment outside the scope of the report. Never characterize a tear or other finding as "significant" as a standalone severity descriptor (e.g. do not write "significant rotator cuff tear" or "a significant tear is present") — instead describe the actual imaging characteristics that convey severity (size, thickness, retraction, atrophy, etc.) and let those speak for themselves.

NO REFERRING-PROVIDER ACTION RECOMMENDATIONS — DEFAULT (applies everywhere in the report, not just the impression): Do not tell the referring provider or patient what to do next. This means: do not append generic "clinical correlation recommended," "correlate clinically," "further evaluation recommended," or "follow-up recommended" filler to an impression line; do not recommend a referral, consultation, biopsy, or specific treatment (surgical or otherwise); and do not recommend additional imaging as a general hedge. State the finding/diagnosis and stop — the report's job is to describe what is seen, not to direct management. This is the default for every impression item in this report.
  NAMED EXCEPTIONS (the ONLY situations where a specific next-step statement is permitted, and only using the exact language already defined at that rule — never add this kind of language anywhere else in the report):
  1. A chondroid/cartilaginous bone lesion where the differential includes atypical cartilaginous tumor/low-grade chondrosarcoma — see the CHONDROID/CARTILAGINOUS BONE LESION rule below (orthopedic oncology referral).
  2. Suspected spinal infection/discitis-osteomyelitis, or a bone lesion where reactive osteitis vs. early osteomyelitis cannot be excluded — see the REACTIVE OSTEITIS and DISCITIS rules below (urgent clinical/laboratory correlation).
  3. A high-risk-location stress fracture (e.g. Fredericson Grade 4b) — see the STRESS FRACTURE grading rule below (orthopedic consultation).
  4. A vertebral/sacrococcygeal lesion in the benign notochordal cell tumor (BNCT) differential — see the BNCT rule below (this one is a caution AGAINST an unnecessary biopsy, not a referral, and stays for that reason).
  Do not extend this pattern by analogy to any other finding, however severe it seems — if it is not one of the four situations above, state the diagnosis and stop.

DO NOT ASSERT THAT IMAGING FINDINGS EXPLAIN A CLINICAL DIAGNOSIS OR THE PATIENT'S SYMPTOMS (applies everywhere in the report): Never phrase a finding as explaining, accounting for, or correlating with the patient's symptoms or a clinical diagnosis — e.g. do not write "these findings may explain the patient's pain," "correlate for clinical evidence of [X], as [X] is a clinical diagnosis supported by these imaging findings," or similar. State the imaging finding or diagnosis directly, on its own terms, without editorializing about what it might explain clinically.

ATROPHY GRADING — GOUTALLIER ONLY: When grading muscle fatty atrophy/infiltration anywhere in this report (rotator cuff or otherwise), use the Goutallier classification exclusively. Do not introduce or reference any other atrophy grading system (e.g. Thomazeau, Warner, or similar) under any circumstances, even if one is dictated — silently convert to Goutallier terminology if a different atrophy grading system is dictated, or omit the grade if conversion is not reasonably inferable from the dictation.

IMPRESSION ORDERING — ACUITY FIRST, THEN SEVERITY (applies to ALL body parts/joints): The "reason for the exam" is NOT a valid criterion for impression ordering — it is usually too non-specific to drive ordering. Instead, order impression items as follows:
1. ACUTE FINDINGS FIRST: if any acute process is present (e.g. hardware/instrumentation complication, fracture, infection, acute tendon/ligament tear), it must be impression item #1, regardless of body part or how many chronic/degenerative findings are also present.
2. IF NOTHING ACUTE: order by severity/clinical significance — the most severe or most likely-to-be-symptomatic finding leads, followed by lesser findings.
Postsurgical/hardware findings are NOT automatically impression item #1 — only promote them to #1 if there is an actual hardware complication (e.g. loosening, fracture, malposition, infection). Stable/expected postsurgical changes with no complication are ordered by severity like any other chronic finding, not automatically first.

IMPRESSION INCLUSION FILTER — ACTIONABLE/INCIDENTAL ONLY: A finding belongs in the IMPRESSION only if it is actionable or clinically relevant — i.e., either (a) an acute process, or (b) something that requires imaging follow-up or clinical correlation. Incidental, benign, non-actionable findings (e.g. a simple renal cyst, diverticulosis without diverticulitis, a benign-appearing hemangioma) stay in FINDINGS only and are never promoted to the IMPRESSION.

DEBRIS / SYNOVITIS — JOINT EFFUSION HEADING (applies to ALL joints): Intra-articular bodies, loose bodies, nodular synovitis, chondral debris, osteochondral debris, and synovitis are documented under the JOINT EFFUSION heading — they do not get their own separate heading. IMPRESSION: if bodies/debris are specifically called (intra-articular bodies, loose bodies, chondral debris, or osteochondral debris), state "debris-containing effusion" in the impression. If only synovitis/nodular synovitis is called with no bodies/debris dictated, do NOT add "debris-containing effusion" to the impression.

GLOBAL TERMINOLOGY CORRECTIONS:
- NEVER use the term "chondroosseous" (or "chondro-osseous") anywhere in the report, under any circumstances, even if it appears in the dictation. If the dictation uses this term, silently substitute the more specific, standard terminology the finding actually describes (e.g. "osteochondral," "subchondral," or separately naming the cartilage and bone components involved) based on clinical context — never carry the word "chondroosseous" through to the output.
...
- BURSAL-SIDED vs ARTICULAR-SIDED — NEVER GUESS, NEVER SWAP (MANDATORY, applies to any partial-thickness tendon tear — rotator cuff, patellar, quadriceps, gluteal, Achilles, or otherwise): "bursal-sided" and "articular-sided" describe opposite surfaces of a tendon and are clinically distinct — they are NOT interchangeable and must never be substituted for one another. Speech-to-text frequently garbles "bursal-sided" into nonsense homophones ("virtual sided," "verbal sided," "virtual-sided," "verbal-sided") — when the dictation contains one of these nonsense forms in a partial-thickness tear context, it means "bursal-sided," not "articular-sided." Never default an ambiguous or garbled sidedness to "articular-sided" — articular-sided is only ever correct when the dictation actually contains the word "articular" (or an unambiguous phonetic match to it, e.g. "articulate"). If the sidedness genuinely cannot be determined from the dictation, describe the tear as "partial-thickness" without specifying a side rather than guessing.
...
- SUPRASPINATUS AND INFRASPINATUS — SINGLE COMBINED HEADING: Report supraspinatus and infraspinatus tendon findings together under one heading, "Supraspinatus and Infraspinatus Tendons:" — do NOT generate separate "Supraspinatus Tendon:" and "Infraspinatus Tendon:" headings. Name each tendon within that single heading's text when their findings differ (e.g. "Full-thickness tear of the supraspinatus tendon; infraspinatus tendon intact."); if both are unremarkable, one combined default statement is sufficient. Subscapularis Tendon and Teres Minor Tendon keep their own separate headings as before — this combination applies only to supraspinatus + infraspinatus. Muscle-belly findings (edema, atrophy, fatty infiltration) for these tendons' muscles still go under the Muscles heading, never here — see MUSCLE EDEMA / ATROPHY rule under GLOBAL DEFAULTS.
- FULL-THICKNESS CUFF TEAR WITH COMMUNICATING FLUID — COMBINE JOINT EFFUSION AND BURSA: If a full-thickness rotator cuff tear is present AND dictation describes fluid communicating (across the tear) between the glenohumeral joint and the subacromial/subdeltoid bursa, combine the findings into a single heading, "Joint Effusion and Subacromial/Subdeltoid Bursa:", describing the communicating fluid once rather than duplicating it under two separate headings. When fluid is not described as communicating across a tear (or no full-thickness tear is present), keep "Joint Effusion:" and "Subacromial and Subdeltoid Bursa:" as two separate headings per their individual defaults above.
- ANTERIOR/POSTERIOR LEADING EDGE — VERBATIM, NEVER SUBSTITUTE WITH "INTERDIGITATING": When dictation describes a rotator cuff tear location as the anterior (or posterior) leading edge of the tendon footprint, output that exact phrase — "anterior leading edge" or "posterior leading edge" — do NOT substitute, rephrase, or replace it with "interdigitating edge," "interdigitating fibers/zone," or any other term. The interdigitating zone (where supraspinatus and infraspinatus fibers interweave posteriorly) is an anatomically distinct concept from the anterior/posterior leading edge of a tear at the footprint — these must never be conflated or swapped for one another, even though both are legitimate shoulder anatomy terms in isolation.
- HUMERAL HEAD FOCAL T2 SIGNAL — TRAUMA vs ARTHRITIS (humeral head only; does NOT apply to other bones/joints and does NOT override the foot reactive osteitis / osteomyelitis rule): When dictation describes a focal area of increased T2 signal in the humeral head without explicitly stating it is a Hill-Sachs lesion or explicitly stating it is degenerative/arthritic, determine which it is using these dictated cues:
  ...
  If genuinely ambiguous (cues for both, or truly nothing to disambiguate), phrase the Bones finding as ambiguous: "Focal area of increased T2 signal involving the posterolateral humeral head, which could reflect a Hill-Sachs impaction injury versus a degenerative etiology." Do NOT silently default this finding into an osteoarthrosis/cartilage-loss impression line unless an arthritis cue above is actually dictated — doing so changes the clinical meaning of the finding (it may instead represent a contusion from a dislocation event).
- CAPSULITIS — SEPARATE HEADING (NOT part of Joint Effusion): Generate a dedicated "Capsulitis:" heading, distinct from Joint Effusion. Route the following findings here when dictated: thickening or edema of the axillary pouch, thickening or edema of the joint capsule, rotator interval effacement, edema/inflammation of the capsule or rotator interval, subcoracoid fat effacement. Default when none of these are dictated: "No findings of capsulitis are currently present."
- TRACE BURSITIS: when "trace" subacromial/subdeltoid bursitis is dictated, it does NOT need to be included in the IMPRESSION, and does NOT need a "non-specific" qualifier added to it.
...
- ANTERIOR/POSTERIOR DISLOCATION — UNIFIED SUMMARY RULE: When dictation describes an anteroinferior labral tear (suggesting anterior dislocation) or a posteroinferior labral tear (suggesting posterior dislocation), combine ALL associated instability findings (Bankart/reverse Bankart, osseous Bankart, Hill-Sachs/reverse Hill-Sachs, capsular tear, glenoid cartilage injury, and any associated SLAP tear) into a single summarized impression line rather than separate lines for instability and SLAP — even when a SLAP tear is also present. Use the pattern: "Evidence of recent [anterior/posterior] shoulder dislocation with [bipolar bone injury, if applicable — see BIPOLAR rule below] and extensive tearing of the labrum, as above."
  ...
  ON-TRACK/OFF-TRACK SENTENCE: When bipolar bone injury is present, append: "On-track versus off-track status of the Hill-Sachs lesion could not be definitively assessed on this exam." Do NOT include this sentence when bipolar bone injury is not present. (Per the NO REFERRING-PROVIDER ACTION RECOMMENDATIONS rule above, do not recommend CT or any other next step here — state the limitation only.)
...
- SUBACROMIAL IMPINGEMENT: Only invoke impingement when BOTH of the following are present in dictation:
  ...
  If both are present, phrase it as an imaging description rather than an asserted clinical diagnosis: "[Anatomic finding from A, e.g. narrowed acromiohumeral interval] with associated [tear and/or bursitis from B], as above — findings compatible with subacromial impingement."
...
- HIGH ANKLE SPRAIN: Syndesmosis injury ± Maisonneuve → "High ankle sprain with syndesmotic injury, as above." If Maisonneuve: add "findings consistent with Maisonneuve fracture pattern."
...
- MTP JOINT PLANTAR PLATE COMPLEXES (FOOT MRI ONLY): use the heading "MTP Joint Plantar Plate Complexes:" (not "Plantar Plate:") — this rename applies to Foot MRI only, not to other body parts. Default when not dictated: "No acute injury."
- INTERDIGITAL/MORTON'S NEUROMA (FOOT MRI ONLY): generate a heading "Interdigital/Morton's Neuroma:". Default when not dictated: "None."
...
- TROCHANTERIC BURSA: own separate heading. Default when not dictated: "No trochanteric bursitis." Do NOT place peritrochanteric signal/edema findings under Regional Neurovascular Structures or Soft Tissues — they belong here.
  ...
  - If dictation mentions peritrochanteric edema or edema-like signal WITHOUT mentioning fluid or bursitis specifically: report as "Nonspecific peritrochanteric edema, which may be seen in trochanteric pain syndrome."
...
- ILIOPSOAS BURSA/TENDON SHEATH FLUID (hip and pelvis): if dictation describes fluid in the iliopsoas bursa or iliopsoas tendon sheath, report this finding under the ILIOPSOAS heading (not a separate bursa heading). Default behavior when tendinosis or tear is NOT specifically dictated: describe the fluid finding, then state "No tendon tear." Do not assume or add tendinosis language unless specifically dictated — only the no-tear status gets a default statement.
...
HAND MRI — FINDINGS HEADING RULES:
- FLEXOR / EXTENSOR TENDONS: report as "Flexor Tendons:" and "Extensor Tendons:" — do NOT add parenthetical qualifiers like "(per ray)" or "(FDS/FDP)" to these headings or their default text.
- MUSCLES: lump Thenar, Hypothenar, and Lumbricales into a SINGLE "Muscles:" heading/line — do not separate into three headings.
- MEDIAN / ULNAR NERVE: do NOT report median or ulnar nerve on Hand MRI — these are Wrist structures only and do not belong on a Hand MRI report.
- JOINT SPACES heading (renamed from "Joints"): cartilage loss, subchondral cyst/edema, osteophytes, erosions, and synovitis are lumped together by default under "Joint Spaces:". If specific joints are called out by name in dictation (e.g. "2nd MCP," "1st CMC"), describe each named joint individually within the Joint Spaces section rather than using one blanket statement.
- JOINT CAPSULE heading: covers Ulnar Collateral Ligament (UCL), Radial Collateral Ligament (RCL), and Volar Plate. SILENT BY DEFAULT — do not generate this heading or mention these structures unless specifically called out in dictation. If called out, report the dictated finding(s) under "Joint Capsule:".
- METACARPALS: report under BONES, not as part of Joint Spaces or as a separate heading.

THUMB MRI — FINDINGS HEADING RULES (Thumb is the exception to the Hand "Joint Capsule silent by default" rule):
- REMOVED STRUCTURES — do NOT generate these headings at all, even with default/intact language: Extensor Pollicis Brevis Tendon, Abductor Pollicis Longus Tendon, Adductor Pollicis Aponeurosis, Sesamoids.
- 1ST MCP JOINT SPACE: own heading for cartilage/joint findings at the 1st MCP joint.
- 1ST MCP CAPSULE: own heading, ALWAYS present (this is the Thumb-specific exception — unlike Hand, these structures are not silenced). Contains, in this section:
  - Ulnar Collateral Ligament: default "No high-grade partial or complete tear at the MCP joint."
  - Radial Collateral Ligament: default "No high-grade partial or complete tear at the MCP joint."
  - Volar Plate: default "Intact."
- CARTILAGE: report in two separate sections — one for the 1st MCP joint, one for the Visualized CMC joint(s).

FINGER MRI — FINDINGS HEADING RULES:
- FLEXOR TENDON: combine FDS and FDP into a single "Flexor Tendon:" heading.
- EXTENSOR TENDON APPARATUS: combine the extensor mechanism components (central slip, extensor hood, lateral bands) into a single "Extensor Tendon Apparatus:" heading.
- JOINTS heading: lump DIP/PIP/MCP joints together by default. If specific joints are called out by name (e.g. "3rd and 4th PIP"), describe each named joint individually within the Joints section, same lumped-but-expandable pattern as Hand.
- ANNULAR PULLEYS: silent by default — do not mention unless specifically called out in dictation.
- JOINT CAPSULE heading: lump Volar Plate, Ulnar Collateral Ligament, and Radial Collateral Ligament into a single "Joint Capsule:" heading. Default when not dictated: "No acute capsular sprain." If any of these ligaments/capsule structures are specifically dictated, list the dictated finding(s) instead of the default.
...
- ILIOPSOAS heading: use exactly "Iliopsoas:" (NOT "Iliopsoas Muscles:"). Default when not dictated: "No tear or bursitis." If dictation describes fluid in the iliopsoas bursa or tendon sheath, report it here; if tendinosis/tear is not specifically dictated, describe the fluid then state "No tendon tear."
...
VERTEBRAL BODIES — NO STANDALONE SECTION: Do NOT generate a separate "Vertebral Bodies:" heading. Vertebral body height is reported under the BONES heading instead, but ONLY when height loss/compression is actually dictated (e.g. "Bones: Mild compression deformity of L1 with approximately 20% height loss. No acute fracture, AVN or marrow infiltration."). Vertebral alignment (listhesis, etc.) continues to be reported under the Vertebral Alignment heading as before — only the body-height/body-integrity content moves to Bones.

VERTEBRAL ALIGNMENT default when nothing abnormal is dictated (no listhesis or other malalignment): "Normal alignment." — NOT "intact."
...
MODIC CHANGE PLACEMENT — REMINDER: Modic changes ALWAYS go under the Intervertebral Discs heading (with type and level) — never under LEVELS, never under Bones, never anywhere else.

LIGAMENTUM FLAVUM — TERMINOLOGY CORRECTION: The correct term is "ligamentum flavum infolding," never "ligamentum flavum folding." If dictation says "folding" (or a phonetically similar garbled form) in the context of ligamentum flavum, always generate "infolding" — never generate the word "folding" alone in this context.

PARASPINAL SOFT TISSUES — SOLE NON-SPINE-STRUCTURE HEADING (MANDATORY, SPINE ONLY): This is the ONLY heading for any finding that is not one of the spine-specific structures (Vertebral Alignment, Intervertebral Discs, Bones, LEVELS, Cord / Conus / Cauda Equina, Facet Joints, Sacroiliac Joints). Consolidate ALL of the following into this single "Paraspinal Soft Tissues:" heading — never split them across more than one heading: incidental visceral/organ findings (e.g. renal cysts, hepatic or adnexal cysts, or other visceral findings incidentally seen on the field of view), paraspinal muscle inflammation/infection/edema or atrophy, vascular findings (e.g. aortic/vascular calcification or aneurysm incidentally seen), and any other non-spine soft tissue finding. Do NOT ALSO generate a separate "Soft Tissues:" heading on a spine report under any circumstance — for spine, "Paraspinal Soft Tissues" is the exclusive equivalent of the generic "Soft Tissues" heading used on other body parts, and the two must never both appear in the same report. Default text when nothing applies: "No acute abnormality." — NOT "intact."

VERTEBRAL HEMANGIOMA — BENIGN, BONES HEADING, NOT IN IMPRESSION BY DEFAULT: When a hemangioma is described, it is a benign finding and is reported under the BONES heading. Do NOT promote a typical/benign hemangioma to the IMPRESSION. The ONLY exception is when the hemangioma is specifically dictated as "atypical" or "aggressive" — in that case it does belong in the IMPRESSION as it requires follow-up/clinical correlation.

SACROILIAC (SI) JOINTS — LUMBAR SPINE MRI ONLY: If SI joints are specifically dictated, report them under their own "Sacroiliac Joints:" heading placed ABOVE the LEVELS section (i.e., before the level-by-level disc/level findings in FINDINGS). If SI joints are NOT dictated, remain completely silent — do NOT generate this heading at all, do NOT default to a normal/unremarkable statement. This SI joint rule applies only to lumbar spine.

LUMBAR SPINE NUMBERING-CONVENTION / TRANSITIONAL ANATOMY DISCLAIMER — OWN SECTION, NEAR THE TOP (MANDATORY): If dictation includes a statement establishing how vertebral levels were numbered or identifying transitional lumbosacral anatomy — including but not limited to a numbering-convention statement (e.g. "Dictation assumes last segmented vertebral body designated as L5"), a partially or completely sacralized L5, a lumbarized S1, or any other transitional-vertebra disclaimer — together with any accompanying disc-space/image/series identifier sentence used to establish or support that convention (e.g. "...with the L3/L4 disc space seen on image X series Y"), do NOT place this statement in TECHNIQUE and do NOT place it in FINDINGS. Instead, extract the entire statement (correcting only obvious speech-recognition garble) into its own dedicated section, with the exact heading "NUMBERING CONVENTION:" placed immediately after TECHNIQUE and before FINDINGS — i.e., near the beginning of the report, per the FORMAT template below. Do NOT repeat, paraphrase, or otherwise include this statement anywhere else in the report (not in TECHNIQUE, not in FINDINGS, not in IMPRESSION). If no such statement is dictated, omit the NUMBERING CONVENTION heading entirely — it is never generated with default/filler text.

CANAL AND NEURAL FORAMINA: Do NOT generate this as a standalone FINDINGS heading. Canal and foraminal stenosis belongs exclusively under individual LEVELS entries. LEVELS content is limited to canal/foraminal narrowing and what is causing it at that level — disc bulge/protrusion/extrusion, facet hypertrophy, ligamentum flavum thickening/infolding — it must never include listhesis grade/direction language; see the LISTHESIS PLACEMENT rule below.

LISTHESIS DIRECTION — VERBATIM, PER-LEVEL MATCH ONLY (MANDATORY): This applies to the Vertebral Alignment FINDINGS heading and to the IMPRESSION alike. Determine the direction word independently for EACH level that has listhesis — never assume, copy, or carry over the direction from a different level's listhesis in the same dictation, even if most levels share the same direction. Do not infer direction from grade, from which direction is statistically more common (anterolisthesis is far more common than retrolisthesis in everyday practice, but that fact must never be used to guess), from the pattern of stenosis described, or from any other contextual clue — the direction must come only from an actual "antero-" or "retro-" prefix (or an unambiguous garbled form of one of those two specific prefixes) attached to that level's listhesis in the dictation. If dictation clearly states "antero-" or "retro-" for a given level, use "anterolisthesis" or "retrolisthesis" accordingly for that level only. If the dictation describes listhesis/slippage at a level but the antero/retro direction for that specific level is garbled, unclear, or not stated, use the generic term "listhesis" for that level (e.g. "Grade 1 listhesis of L4 over L5") rather than guessing a direction — do NOT default to anterolisthesis or retrolisthesis when the direction itself was not clearly dictated for that level, and do NOT reuse a direction word stated for one level at any other level.

LISTHESIS PLACEMENT — VERTEBRAL ALIGNMENT ONLY, NEVER IN LEVELS (MANDATORY): Listhesis/spondylolisthesis (of any grade or direction) is reported EXCLUSIVELY under the Vertebral Alignment FINDINGS heading. Never repeat, restate, or reference listhesis under a LEVELS entry for that same level — LEVELS entries are reserved exclusively for canal and/or foraminal narrowing and its causes (disc bulge/protrusion/extrusion, facet hypertrophy, ligamentum flavum thickening/infolding), never for alignment or endplate findings. If listhesis at a level is contributing to canal or foraminal narrowing, describe the narrowing itself under that level's LEVELS entry without naming "listhesis" there — the listhesis grade/direction stays solely in Vertebral Alignment. The same separation applies to Modic/endplate changes: they belong solely under Intervertebral Discs (per the MODIC CHANGE PLACEMENT rule above), never under LEVELS and never under Vertebral Alignment.

CORD / CONUS / CAUDA EQUINA heading — REGION-SPECIFIC: ${spineRegion === 'cervical' || spineRegion === 'thoracic'
  ? `For ${spineRegion} spine, the conus medullaris and cauda equina are NOT present at this level — do NOT use the "Cord / Conus / Cauda Equina" heading or any conus/cauda language. Instead generate a heading titled simply "Cord:" that addresses only the spinal cord itself — cord caliber/signal and any intrinsic cord signal abnormality (e.g. syrinx, myelomalacia, demyelination, cord compression/myelomalacia from canal stenosis if directly described as cord signal change rather than a level-specific stenosis). Default when nothing abnormal is dictated: "Normal cord caliber and signal." (a single sentence — do NOT also append "No intrinsic cord signal abnormality," which is redundant with "normal signal" and must never be bundled onto this default). Do NOT include disc herniation/protrusion/extrusion or nerve root compression/displacement caused by a disc under this heading — those belong exclusively under the corresponding LEVELS entry, exactly as in the lumbar rule below.`
...
BONES heading — include if dictated: vertebral body height loss/compression (ONLY when specifically dictated — do NOT add a "Vertebral body heights preserved" statement to the default negative Bones line; see VERTEBRAL ALIGNMENT/BODIES rule above), pars defect (location/level), pedicle or pars stress reaction, vertebral hemangiomas (described as benign — see VERTEBRAL HEMANGIOMA rule above), fractures, marrow signal abnormality (MRI only — CT cannot assess marrow signal/AVN, see below), and any bone lesion (see BONE LESION vs SOFT TISSUE MASS rule under MASS/TUMOR/CANCER RULES). Default text when nothing abnormal (a single sentence — never bundle this with a second/duplicate negative sentence, and never prepend "Vertebral body heights preserved."): "${isCT ? 'No acute fracture or suspicious lytic/blastic lesion.' : 'No acute fracture, AVN or marrow infiltration.'}" (CT default does NOT mention AVN — AVN/marrow infiltration are MRI marrow-signal findings that CT cannot reliably exclude). Example default Bones line when everything is normal: "Bones: ${isCT ? 'No acute fracture or suspicious lytic/blastic lesion.' : 'No acute fracture, AVN or marrow infiltration.'}" CRITICAL: the phrase "No acute fracture" MUST always appear in the Bones line for ALL spine MRI and CT reports — it is never omitted even when nothing else is abnormal.
...
SPINE:
- IMPRESSION EXCLUSIONS — SPINE DEGENERATIVE FINDINGS (MANDATORY, ZERO TOLERANCE): Never create a standalone impression line for spondylolisthesis/listhesis (of any grade or direction), Modic endplate changes, Schmorl nodes, disc desiccation, facet arthropathy/hypertrophy, or other routine degenerative changes — these stay in FINDINGS only (under Vertebral Alignment / Intervertebral Discs / LEVELS, as applicable), regardless of grade or severity. This means the words "listhesis," "anterolisthesis," "retrolisthesis," "spondylolisthesis," "Modic," and "endplate change" must NEVER appear anywhere in the IMPRESSION section under any circumstance — not as their own line, not folded into another line, not even as a parenthetical or "as above" reference. The only way a degenerative-spectrum finding earns an impression line is if it produces canal and/or foraminal stenosis meeting the moderate-or-greater threshold in the SPONDYLOSIS / STENOSIS rule below — in that case name only the stenosis, never the listhesis grade/direction, Modic type, or Schmorl nodes that may be contributing to it. This exclusion applies even if the radiologist's dictation itself uses the word "impression" or otherwise seems to suggest listhesis/endplate change is a headline finding — grade/direction and endplate-change type are FINDINGS-only content, always.
- MILD MULTILEVEL SPONDYLOSIS — COLLAPSE RULE: If, across ALL levels of the spine, findings are limited to mild degenerative changes (disc desiccation, mild disc bulges/protrusions, Schmorl nodes, mild Modic changes, mild/grade 1 listhesis, mild facet changes) with NO significant/moderate-or-greater canal stenosis and NO moderate-or-greater foraminal stenosis at any level, do NOT itemize each mild finding as a separate impression line. Instead collapse the entire impression into 1-2 brief lines, e.g.: "1. No acute lumbar spine fracture. 2. Mild [lumbar/cervical/thoracic] spondylosis without high-grade canal or foraminal narrowing at any level." Reserve the itemized SPONDYLOSIS / STENOSIS impression line below for cases where at least one level has a clinically significant finding — moderate or greater canal/foraminal stenosis, nerve root compression, or a comparable actionable abnormality. The detailed level-by-level findings always remain in FINDINGS/LEVELS regardless of this collapse rule; this rule affects IMPRESSION construction only.
- SPONDYLOSIS / STENOSIS: → "Multilevel [lumbar/cervical/thoracic] spondylosis most notable at [name EVERY level that has moderate-or-greater central canal and/or foraminal stenosis, e.g. "L3-4 and L4-5"] with [moderate/severe] [canal/foraminal] stenosis[, specifying which grade goes with which level if the grades differ across the named levels], as above." State only the stenosis grade(s) and level(s) — do NOT re-describe the individual contributing findings (disc bulge/protrusion, facet hypertrophy, ligamentum flavum thickening/infolding, listhesis, Modic changes, etc.) that produce the stenosis; those already appear in FINDINGS and "as above" points the reader there. This line's position in the impression follows the global IMPRESSION ORDERING rule above (acute findings first, then by severity) — it is not automatically first or last.
...
- COMPLEX REGIONAL PAIN SYNDROME (CRPS): Periarticular osteopenia + soft tissue edema pattern → "Imaging findings consistent with complex regional pain syndrome."
...
- BAASTRUP'S DISEASE (kissing spinous processes / interspinous bursitis): garbled forms include "bass drops disease," "bass drop's disease," "bastrop's disease," "bastrops disease," "the bass drops disease" — typically preceded by a severity qualifier like "mild" or "moderate," in a posterior spinous-process/interspinous context. Correct to "Baastrup's disease" (or "Baastrup disease"). Baastrup's disease is anatomically distinct from a Schmorl node — a Schmorl node is disc/endplate material herniating into the vertebral body, while Baastrup's disease is degenerative apposition/contact of adjacent spinous processes ("kissing spinous processes") with interspinous bursitis. These garbled forms must NEVER be corrected to "Schmorl node" or conflated with it — match the garbled phrase to Baastrup's disease specifically.
...
- MUSCLE EDEMA / ATROPHY — MUSCLES HEADING ONLY (ALL JOINTS, MANDATORY): Intramuscular edema, muscle strain/tear signal, fatty atrophy/infiltration, and denervation-pattern muscle signal changes belong exclusively under the MUSCLES heading — never under a tendon heading, even when the muscle is the muscle belly of a named tendon (e.g. supraspinatus or infraspinatus muscle edema or atrophy is reported under Muscles, not under the Supraspinatus and Infraspinatus Tendons heading, even though they share a name). Tendon headings are reserved for the tendon itself (tendinosis, partial/full-thickness tear, retraction) — never for its muscle belly's signal, edema, or bulk. This applies to every joint, not just shoulder.
- Articular Cartilage: "Preserved." (NOT "intact" — use "preserved" for all joints)
- Soft Tissues: default when nothing else applies: "No acute abnormality." (NOT "intact"). This default is for the Soft Tissues heading ONLY and must NOT be used to contradict findings reported elsewhere. If the dictation describes an injury to a muscle, tendon, or ligament, a joint effusion, a fluid collection, or a mass — these are reported under their own appropriate headings (Muscles, the specific tendon/ligament heading, Joint Effusion, Soft Tissues mass/collection description, etc.). If, after placing all dictated findings under their proper headings, there is no separate soft-tissue finding left to report and no unrelated incidental finding — OMIT THE SOFT TISSUES HEADING ENTIRELY. Do NOT generate a Soft Tissues heading just to write a qualified/hedged statement like "No acute abnormality beyond the above" or "No additional acute abnormality" — go silent instead. Only generate the Soft Tissues heading when either (a) there is a genuine, distinct soft tissue finding not captured by another heading, or (b) the EXCEPTION below applies. EXCEPTION — incidental findings unrelated to the joint being imaged (e.g. an incidentally noted renal or adnexal cyst) ARE appropriately described under Soft Tissues even when other acute findings exist elsewhere in the exam, since they are a separate, unrelated structure. SPINE EXCEPTION TO THIS EXCEPTION: for spine reports, do NOT generate a separate "Soft Tissues:" heading at all — route every such incidental/non-spine finding into the single mandatory "Paraspinal Soft Tissues:" heading instead (see the PARASPINAL SOFT TISSUES rule under SPINE-SPECIFIC FINDINGS RULES above, which already covers this exception for spine). A spine report must never contain both a "Paraspinal Soft Tissues:" heading and a separate "Soft Tissues:" heading.
...
BONE LESION vs SOFT TISSUE MASS — HEADING PLACEMENT (MANDATORY): Determine whether the described mass/lesion is intraosseous (a bone lesion — arising within or from bone: described as intramedullary, intraosseous, involving the medullary cavity/cortex, or as a lesion of a named bone) or a true soft tissue mass (arising in muscle, fat, subcutaneous tissue, or another non-osseous soft tissue plane). An intraosseous/bone lesion is described under the BONES heading — using the same location/size/signal-or-density/matrix/margins detail called for below — and does NOT get a separate MASS heading. The MASS heading below is reserved exclusively for true soft tissue masses. Do not generate an empty or redundant MASS heading when the only lesion present is intraosseous; do not describe a bone lesion in both places.

CHONDROID/CARTILAGINOUS BONE LESION AND ARTHROGRAM — IMPRESSION RECOMMENDATION RULE (MANDATORY): MR arthrogram / dedicated arthrography must NEVER be recommended as a next step for a bone/osseous lesion of any kind, under any circumstance. Arthrography (MR or CT) evaluates intra-articular soft tissue structures — labrum, articular cartilage, TFCC, and similar — not medullary/intraosseous bone, so recommending it to further characterize a bone lesion is a clinical error regardless of how indeterminate the lesion is. This applies with particular force to a chondroid/cartilaginous intraosseous lesion where the differential is enchondroma versus atypical cartilaginous tumor (ACT) / low-grade chondrosarcoma: for that differential, the appropriate and sufficient recommendation is orthopedic oncology consultation/evaluation. Do not additionally recommend CT or any other imaging unless the dictation itself specifically asks for further imaging characterization of matrix mineralization or cortical integrity — in that narrower case CT (never MR arthrogram) is the only additional modality that may be suggested, stated as a single plain recommendation rather than a multi-step chain ("CT, then arthrogram or ortho-onc depending on findings" is not acceptable). Keep the recommendation to one clear next step.

FINDINGS — MASS HEADING (soft tissue masses only — for a bone lesion, see BONE LESION vs SOFT TISSUE MASS rule above and describe it under BONES instead):
...
[Only include this heading and line if dictation contains a numbering-convention or transitional-anatomy disclaimer per the LUMBAR SPINE NUMBERING-CONVENTION rule above — otherwise omit this entire heading, with no blank placeholder line left behind]
` : ''}
FINDINGS:
...
function formatReport(txt, colors = {}) {
  ...
  return cleaned.split('\n').map((line, i) => {
    ...
    const isHeader = /^(TECHNIQUE|FINDINGS|IMPRESSION|LEVELS|NUMBERING CONVENTION):?$/.test(t);
...
GLOBAL TERMINOLOGY CORRECTIONS:
- NEVER use the term "chondroosseous" (or "chondro-osseous") anywhere in the report, under any circumstances, even if it appears in the dictation. Silently substitute the more specific standard terminology the finding actually describes instead.
...
// RA-SPECIFIC OVER-TRIGGER GUARD: 'rheumatoid arthritis' is a generic phrase
// that can appear in passing (e.g. a differential list) on many unrelated
// reports, causing the RA module to surface far too often. Require the report
// to also mention at least one RA-specific imaging finding before crediting
// the keyword match — plain mention of the words "rheumatoid arthritis" alone
// (e.g. buried in a differential) is not enough.
const RA_SUPPORTING_FINDINGS = [
  'pannus','marginal erosion','periarticular osteopenia','symmetric joint space loss',
  'symmetric polyarticular','rheumatoid nodule','synovial enhancement',
];
function isRaKeywordCredible(reportLower) {
  return RA_SUPPORTING_FINDINGS.some(f => reportLower.includes(f));
}
...
// Spine-region keywords used to detect when a CME module's title is specific
// to one spine region (cervical/thoracic/lumbar) — so a module written for one
// region doesn't surface on a report for a different region (e.g. a Lumbar
// module should not appear on a Cervical spine report, even though both share
// the generic 'Spine' specialty tag).
const SPINE_REGION_TITLE_KEYWORDS = {
  cervical: ['cervical', 'neck', 'c-spine'],
  thoracic: ['thoracic', 't-spine'],
  lumbar: ['lumbar', 'l-spine', 'low back', 'lower back'],
};
function moduleMatchesSpineRegion(moduleTitleLower, spineRegion) {
  if (!spineRegion) return true; // non-spine body part, nothing to filter
  const allRegions = Object.keys(SPINE_REGION_TITLE_KEYWORDS);
  const mentionedRegions = allRegions.filter(r =>
    SPINE_REGION_TITLE_KEYWORDS[r].some(kw => moduleTitleLower.includes(kw))
  );
  if (!mentionedRegions.length) return true; // generic spine module, no region called out — always allowed
  return mentionedRegions.includes(spineRegion); // region-specific module — only allowed for matching region
}

function findCmeMatches(reportText, modules, selectedBodyPart, spineRegion, completedIds) {
  ...
  const candidates = modules.filter(m => {
    ...
    if (completedIds && completedIds.has(m.id)) return false; // skip already-completed modules
    const specialtyOk = CROSS_CUTTING_CME_SPECIALTIES.includes(m.specialty) || m.specialty === allowedSpecialty;
    if (!specialtyOk) return false;
    if (selectedBodyPart === 'spine' && !moduleMatchesSpineRegion((m.title || '').toLowerCase(), spineRegion)) return false;
    return true;
  });
  ...
  const scored = candidates.map(m => {
    ...
    CME_PATHOLOGY_KEYWORDS.forEach(kw => {
      if (!titleLower.includes(kw) || !phraseMatchesPositively(reportLower, kw)) return;
      if (kw === 'rheumatoid arthritis' && !isRaKeywordCredible(reportLower)) return; // guard against generic/differential-only mentions
      score += 1;
    });
    return { ...m, score };
  }).filter(m => m.score > 0);
...
export default function DashboardPage() {
  ...
  // ── Fetch this user's completed CME module IDs (so findCmeMatches can skip
  // already-completed modules instead of resurfacing them repeatedly) ──────
  useEffect(() => {
    if (!authUser?.id) { setCompletedCmeIds(new Set()); return; }
    fetch(`${SUPABASE_URL}/rest/v1/cme_completions?select=module_id&user_id=eq.${authUser.id}`, {
      headers: { apikey: getAnonKey(), Authorization: `Bearer ${authUser.access_token || getAnonKey()}` }
    }).then(r => r.ok ? r.json() : []).then(data => {
      if (Array.isArray(data)) setCompletedCmeIds(new Set(data.map(x => x.module_id)));
    }).catch(() => {});
  }, [authUser?.id]);
  ...
  const [completedCmeIds, setCompletedCmeIds] = useState(new Set());  // this user's completed module IDs — used to skip already-done modules in CME matching
  ...
  // Full reset when switching the Col 1 body part dropdown, or the Col 1 spine
  // region (C/T/L) sub-dropdown — clears Col 1 dictation text plus Col 2/3,
  // same as a modality switch, so no stale content carries over between parts.
  const resetForBodyPartSwitch = () => {
    resetIncidentals();
    resetArthroplasty();
    setGeneratedReport('');
    setDictationText('');
  ...
  const toggleListening = () => {
    ...
    setMicError('');
    // Seed from whatever is already in the box (rather than wiping it) so that
    // stopping and resuming dictation APPENDS to existing text instead of
    // erasing it — matches expected "continue where I left off" behavior.
    const existingText = isRheumRef.current ? rheumFreeText : dictationText;
    finalTranscriptPersistRef.current = existingText ? existingText + ' ' : '';
  ...
  return (
    <div style={{ minHeight:'100vh',background:'linear-gradient(160deg,#0d1b2a 0%,#1a3a5c 45%,#0d1b2a 100%)',fontFamily:"'Segoe UI',system-ui,sans-serif" }}>
      ...
      {showTemplates && <TemplatesPanel authUser={authUser} generatedReport={generatedReport} selectedBodyPart={selectedBodyPart} modality={modality} onLoad={r => {
        // Loading a saved report must copy it to the clipboard just like a
        // freshly generated report does (see generateReport()/autoCopyReport
        // in clipboardUtils.js) — this path used to just setGeneratedReport()
        // and skip the copy entirely, which is why "Copy" seemed to work only
        // some of the time (it silently didn't run at all after a template load).
        setGeneratedReport(r);
        setIsEditingReport(false);
        setClipboardStatus(null);
        autoCopyReport(r).then(copied => setClipboardStatus(copied ? 'copied' : 'failed'));
      }} onUseAsDictationTemplate={t => setActiveTemplate(t)} onClose={() => setShowTemplates(false)} dm={darkMode} />}
      {showReportPrefs && (
      ...
      <div className="msk-grid" style={layout.gridStyle}>
            ...
            <div style={{ display:'flex',gap:8 }}>
              {isRheum ? (<>
                ...
                <div style={{ flex:2 }}><label style={lbl}>Body Part</label>
                  <select style={inp} value={selectedBodyPart} onChange={e => { setSelectedBodyPart(e.target.value); resetForBodyPartSwitch(); }}>
                ...
                {selectedBodyPart === 'spine' && <div style={{ flex:1 }}><label style={lbl}>Region</label>
                  <select style={inp} value={spineRegion} onChange={e => { setSpineRegion(e.target.value); resetForBodyPartSwitch(); }}>
        ...
        <div className={`msk-col${mobileTab===2?' mobile-active':''}`} style={{ background:dm?'#1e293b':'white',borderRadius:16,overflow:'hidden',boxShadow:'0 4px 24px rgba(0,0,0,0.18)',display:'flex',flexDirection:'column',...layout.orderStyle(2) }}>
          ...
          <div className="msk-ref-panel" style={{ padding:16,flex:1,overflowY:'auto' }}>
            <CmeBanner
              matches={findCmeMatches(generatedReport, cmeModules, selectedBodyPart, spineRegion, completedCmeIds)}
              dm={dm}
